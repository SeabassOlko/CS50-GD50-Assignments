        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    ...
                    -- check if thers a match, if so then calculate matches
                    if matches then
                        self:calculateMatches()

                    -- else swap the tiles back and play sound 
                    else
                        gSounds['error']:play()
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            -- go through tile types in match and calculate score for each tile
            for t, tile in pairs(match) do
                -- check if shine and if true add row to score
                if tile.shine == true then
                    for r, tileRow in pairs(self.board.tiles[tile.gridY]) do
                        self.score = self.score + tileRow.variety * 50
                    end
            ...
            -- add time to timer for every match
            self.timer = self.timer + 1
        end
...
function PlayState:render()
    ...
    love.graphics.printf('highlighted block: ' .. tostring(self.highlightedTile), 20, 145, 182, 'center')
    if self.highlightedTile ~= nil then
        love.graphics.printf('highlighted block Grid x: ' .. tostring(self.highlightedTile.gridX), 20, 175, 182, 'center')
        love.graphics.printf('highlighted block grid y: ' .. tostring(self.highlightedTile.gridY), 20, 200, 182, 'center')
    end
end