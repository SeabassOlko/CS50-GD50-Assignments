...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.hit and not self.split then
            self.split = true
            local velX, velY = self.alien.body:getLinearVelocity()
            for i = 1, 2 do
                if i == 1 then
                    table.insert(self.aliens, Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() - 40, 'Player'))
                else
                    table.insert(self.aliens, Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() + 40, 'Player'))
                end
                self.aliens[i].body:setLinearVelocity(velX + (4 / i), velY + (4 / i))
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        self.alien:render()
        for k, aliens in pairs(self.aliens) do
            aliens:render()
        end
    end
end